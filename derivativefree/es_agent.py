...
def es(f, th_mean, batch_size, n_iter, initial_std=1.0):
    """
    ...
    for _ in range(n_iter):
        ths = np.array([th_mean + 0.01 *dth for dth in np.random.randn(batch_size, th_mean.size)])
        ys = np.array([f(th) for th in ths]) # here is the reward for each episode
        delta_ths = np.array([ths[i]*ys[i] for i in range(len(ys))])
        th_mean = th_mean + 0.01*delta_ths.mean(axis=0)
